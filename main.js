...
$(document).ready(function () {
    ...
    chrome.storage.local.get(['lastVersionChecked', 'lastVersionAvailableOnline'], function (result) {
        if (typeof result.lastVersionChecked === undefined || ($.now() - result.lastVersionChecked) > 3600 * 1000) {
            try {
                var url = 'https://api.github.com/repos/uvify/betaflight-configurator/tags';
    ...
    $('a', ui_tabs).click(function () {
        if ($(this).parent().hasClass('active') == false && !GUI.tab_switch_in_progress) { // only initialize when the tab isn't already active
            ...
            GUI.tab_switch_cleanup(function () {
                ...
                switch (tab) {
					case 'currentstate':
                        TABS.currentstate.initialize(content_ready);
                        break;					
					case 'draco':
						TABS.draco.initialize(content_ready);
						break;
                    case 'landing':
                    ...
                    case 'led_strip':
                        ...
                        break;
					case 'arm_led':
						TABS.arm_led.initialize(content_ready);
						break;
                    case 'failsafe':
...
function updateTabList(features) {
	/*
	$('#tabs ul.mode-connected li.tab_setup').show();
	...
	$('#tabs ul.mode-connected li.tab_receiver').hide();
	...
	$('#tabs ul.mode-connected li.tab_motors').hide();
...
*/
	...
	if (isExpertModeEnabled()) {
		...
		$('#tabs ul.mode-connected li.tab_motors').show();
		...
		$('#tabs ul.mode-connected li.tab_arm_led').show();
		...
		$('#tabs ul.mode-connected-cli li.tab_cli').show();		
	}
	else {
		$('#tabs ul.mode-connected li.tab_currentstate').show();
		$('#tabs ul.mode-connected li.tab_ports').hide();
		$('#tabs ul.mode-connected li.tab_configuration').hide();
		$('#tabs ul.mode-connected li.tab_draco').show();
		$('#tabs ul.mode-connected li.tab_failsafe').hide();
		$('#tabs ul.mode-connected li.tab_pid_tuning').hide();
		$('#tabs ul.mode-connected li.tab_receiver').hide();
		$('#tabs ul.mode-connected li.tab_auxiliary').hide();
